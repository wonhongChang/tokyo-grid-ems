...
도쿄전력 파워그리드(TEPCO)가 공개하는 시계열 전력 데이터를 기반으로, 아래 세 가지 기능을 제공하는 **운영(배치) 지향 EMS(에너지 관리) 프로토타입**입니다.

- 전력 수요 **예측** (시간별, 피크 시각/값 포함)
- 예측 대비 **이상 패턴 탐지** (급등/급락, 잔차 드리프트, 공급 예비율 위험)
- GitHub Pages로 공개 가능한 **정적 대시보드**

> 전제: "실시간"이 아니라 **전날까지 데이터가 갱신되는 환경**을 가정합니다.
> 따라서 **어제의 이상 탐지 리포트** + **오늘/내일 예측 리포트**를 중심으로 화면을 구성합니다.
...
| 예측 / 이상 탐지 | Python (통계 베이스라인 → LightGBM 예정) |
| 대시보드 | React + Vite |
...
- **ETL**: TEPCO 월별 ZIP을 매일 다운로드 → 파싱 → JSON 생성 → GitHub Pages 배포
- **Intraday**: 2시간마다 당일 실시간 데이터 취득·갱신
...
| Phase 5-A | LightGBM 예측 모델 (기온 없음) | 설계 완료 |
| Phase 5-B | 기온 데이터 연동 (Open-Meteo) | 설계 완료 |
