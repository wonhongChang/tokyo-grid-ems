...
| intraday | `ramp_guard.observed_drop_relaxation.decline_support` | enabled | Allows a wider final drop cap only when actual demand is already falling and both lag/recent target-hour deltas support a strong decline. Raising the caps preserves sharper evening drops; lowering them keeps the line closer to the latest observed level. |
