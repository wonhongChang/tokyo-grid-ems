...
- OpenAI calls are cost-capped by default: only the latest daily report date is eligible. The default chain uses up to two calls: first an English master analysis from the compact fact packet (`OPENAI_DAILY_REPORT_MODEL`, default `gpt-5.4-mini`), then low-cost `ko`/`ja` localization from that English master (`OPENAI_DAILY_REPORT_LOCALIZATION_MODEL`, default `gpt-4o-mini`). `OPENAI_DAILY_REPORT_MAX_CALLS_PER_RUN=2` remains the default. Use `OPENAI_DAILY_REPORT_LOCALES`, `OPENAI_DAILY_REPORT_MAX_CALLS_PER_RUN`, and `OPENAI_DAILY_REPORT_LATEST_ONLY` to deliberately narrow or widen that scope.
