...
- `summary.modelWins`, `summary.tepcoWins`: 絶対誤差基準の時間別勝敗
- `daily[]`: 日別MAEと勝敗。`includedInSummary: false` の日付は全体サマリーから除外
- `daily[].maeGapMw`: モデルMAEからTEPCO MAEを引いた値。正ならTEPCOが近く、負ならモデルが近い
- `daily[].verdict`: `model_better`, `tepco_better`, `close`, `insufficient` のいずれかの日別判定
- `hourly[]`: 時間帯別MAEと勝敗
