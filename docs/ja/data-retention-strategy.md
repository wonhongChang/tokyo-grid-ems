...
TokyoGridEMSはGitHub Pagesで静的JSONファイルを公開します。GitHub ActionsがTEPCO/JMA気象データを収集し、JSON/parquet/model成果物を作成し、GitHub Pagesがバックエンドなしでそれらを配信します。
...
lead-time予測スナップショットは、ETLとintraday実行時に `forecast_snapshots/` 配下へ保存します。

- 直近21個のtarget dateを保持します。
- target dateごとに最大16件のスナップショットを保持します。
...
1. ETLの最後に古い `actual`、`forecast`、`alerts` 日次JSONを `archive/{actual,forecast,alerts}/YYYY-MM.json` へ圧縮するcleanupを追加します。
2. archive作成後、直近日次JSONだけを残します。
3. UIで過去月を探索する必要が出た場合、archive month indexファイルを追加します。
4. forecast snapshot履歴が大きくなった場合、古いスナップショットをcompact lead-time metricsへ変換します。
