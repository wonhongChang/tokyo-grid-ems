...
|---|---|---|
| Spike | 実績が `p99Upper` を超え、超過幅が warning の MW または % 基準以上 | 実績が `p99Upper` を超え、超過幅が critical の MW または % 基準以上 |
| Drop | 実績が `p99Lower` を下回り、超過幅が warning の MW または % 基準以上 | 実績が `p99Lower` を下回り、超過幅が critical の MW または % 基準以上 |

p95 だけを少し外れた場合は spike/drop イベントにしません。p99 をごく小さく外れた場合も、運用上意味のある超過幅でなければ除外します。これは運用上の急騰/急落というより通常のモデルバンド誤差に近く、同じ方向に複数時間続く場合は drift 検知が別途拾います。
