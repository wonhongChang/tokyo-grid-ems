...
- `correction`: residual補正メタデータ。日付境界carry-over、日単位bias、`businessTypeTransitionPriorApplied`、`businessTypeTransitionPriorBiasMw`、`businessTypeTransitionApplied`、`businessTypeTransitionBiasMw` などの営業/非営業遷移補正フラグに加え、`positiveResidualMitigationApplied`、`positiveResidualMitigationMaxMw` などのhandoff緩和フィールドを含む場合があります
