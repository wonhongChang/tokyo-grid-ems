...
2026-09-08訂正: 以下の区間実験は最終表示線に基づく過去資料の検証であり、従来の「固定origin」という表記を訂正した。発行時に保存した長いleadの区間には包含率不足があり、[9月8日レビュー](../model-reviews/model-review-2026-09-08.md)に修正課題を記録した。

## 問題

従来のrolling conformalは予測区間の**最小幅を引き上げるfloor**に過ぎなかった。native quantileが過大でも縮小できず、v14-r2最終表示線12日ではP95 coverageが100%である一方、平均半幅は3,700.2MWに達し、大半の時間が3,750MW上限に張り付いた。時間帯ごとのリスクを区別できない状態だった。
...
以下はtarget日をcalibration履歴から除外し、最終表示線を評価した過去資料のwalk-forward結果である。発行時入力の完全な再現やlead別の独立holdout検証ではない。同じ履歴はポリシー選択にも使われており、未使用テストセットの性能とは解釈できない。

| 期間 | 従来P95 coverage | 新coverage | 従来平均半幅 | 新平均半幅 | 変化 |
|---|---:|---:|---:|---:|---:|
| v14-r2最終表示線12日 | 100.00% | 98.61% | 3,700.2MW | 2,455.5MW | 33.6%縮小 |
...
- 最終表示残差から求めた幅をすべてのleadに適用する妥当性は未確認である。9月8日に確認した長いleadの不足は別途修正が必要となる。
