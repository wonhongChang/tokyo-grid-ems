...
- 各root-cause hypothesisが原因メカニズムと次の確認点を明示する。
- freeze policyの説明は実際のfreeze gapがある場合だけ出る。
- 改善候補は実在するfeatureまたはpost-processing layerを対象にする。
- 改善候補は単なるreview文ではなく、検証窓、threshold、guard、replay対象を含む。
...
### 事象文脈と提案の根拠 (2026-09-16)

- 朝という時刻だけで営業区分遷移を原因にしません。既定の調査対象は`lag_24h_hourly_delta`で、同区分anchor・気象・元予測・補正値を比較します。遷移専用の説明には`lag24BusinessTypeMismatch == 1`が必要ですが、この値も因果関係の証明ではありません。
- 夕方低下capの自動候補は、正の予測誤差、`sameDayActualSlopeMw <= -500`、`postCalibrationForecastDeltaMw > 0`がそろう場合に限ります。既存のshape診断基準による**レポート候補選別**であり、新しいモデル制御規則ではありません。欠損や時刻だけでは生成しません。
- 診断値と原因確定を分けます。最終実行のslopeだけでは、それ以前の実行が低下を知っていたとは言えません。調整には対象時間開始前の入力によるreplayが必要です。
- 生成器が補う`mechanism`、`nextCheck`と夕方候補の説明も韓国語・日本語に対応します。十分なAI説明を汎用テンプレートへ置き換えません。
- レポート生成CLIはモデルreplayではないため、`proposedReplayCommand`として提示しません。実際の実験が定義されるまでコマンドを省略します。

検証: `tests/test_ai_report_context_safety.py`、`tests/test_ai_report_ticket_evidence.py`。[9月16日レビュー](model-reviews/model-review-2026-09-16.md)ではネットワーク遮断テスト74件と実入力の三言語文脈確認が通過しました。既存公開レポートは再生成せず、有料API検証は別途承認された呼び出し予算内でのみ実施します。
