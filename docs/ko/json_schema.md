  ...
  "rootCauseHypotheses": [
    {
      ...
      "mechanism": "전날 영업일 lag가 당일 실측이 충분히 쌓이기 전 비영업일 오전 수요를 과도하게 끌어올릴 수 있습니다.",
      "nextCheck": "06~11시 구간 replay에서 lag_24h, recent_same_business_type_mean, intraday 보정 delta를 비교합니다.",
...
- `rootCauseHypotheses[].mechanism`은 원인 경로를 설명하고, `nextCheck`는 코드 수정 전에 확인할 replay, 필드, 진단 자료를 명시합니다.
- `rootCauseHypotheses[].sourceEventIds`는 자연어 가설을 `analysisPriorities.events`의 원천 이벤트와 연결합니다.
...
- OpenAI 호출은 기본적으로 비용 제한을 둡니다. 최신 일일 리포트 날짜만 OpenAI 후보이며, 기본 체인은 저비용 모델을 사용합니다. 먼저 압축된 fact packet으로 영어 마스터 분석을 만들고(`OPENAI_DAILY_REPORT_MODEL`, 기본값 `gpt-4o-mini`), 그 영어 마스터를 기준으로 `ko`/`ja`를 현지화합니다(`OPENAI_DAILY_REPORT_LOCALIZATION_MODEL`, 기본값 `gpt-4o-mini`). 범위를 줄이거나 넓히려면 `OPENAI_DAILY_REPORT_LOCALES`, `OPENAI_DAILY_REPORT_MAX_CALLS_PER_RUN`, `OPENAI_DAILY_REPORT_LATEST_ONLY`를 명시적으로 조정합니다. 더 강한 분석 모델이 필요하면 `OPENAI_DAILY_REPORT_MODEL`을 지정합니다.
...
- OpenAI 입력에는 전체 시간별 진단 row를 넘기지 않고 압축된 fact packet만 전달합니다. fact packet에는 `coverageContext`, `controllerDiagnosis`, `stageAttribution`, `bandQuality`, `freezeImpact`, `rollingPatternContext`처럼 Python이 계산한 판정/요약 필드를 포함합니다. 프롬프트 입력에서는 fallback 자연어 객체, 규칙 기반 `insights`, 파일 path, SHA-256 fingerprint, `performance`와 중복되는 summary 블록을 제외합니다. deterministic 지표, 입력 경로, 데이터 품질, `inputSnapshot`은 Python 코드가 고정합니다.
