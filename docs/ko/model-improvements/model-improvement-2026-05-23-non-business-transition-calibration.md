...
이 레이어는 실측 기반 전환 보정보다 훨씬 약합니다. 이제 단순 실측 개수만으로 끄지 않고, `lastObservedHour < 6`인 동안에는 평가 기회를 유지합니다. `lastObservedHour >= 6`이 되면 실측 기반 전환 보정으로 넘기기 위해 무조건 꺼집니다.
...
2026-05-23 라이브 실행에서는 이른 아침 ramp 구간의 handoff 공백이 드러났습니다. 07:44 JST 시점에는 실측이 5개 있었지만 마지막 실측 시간은 04시였습니다. 기존 로직은 실측 개수가 intraday 최소 기준에 도달했다는 이유로 prior를 껐고, 동시에 `lastObservedHour < 6`이라 실측 기반 전환 보정도 아직 켜지지 않았습니다.

새 로직은 이 공백 구간에서 prior의 평가 기회를 유지합니다. 또한 다음 조건을 모두 만족할 때만 새벽의 작은 양수 residual이 과열된 주말 아침 ramp 예측을 더 끌어올리지 못하게 제한합니다.

- 대상일이 비영업일이고 24시간 lag가 다른 영업/비영업 타입에서 왔습니다.
- `lag_24h`가 최근 같은 비영업일 anchor보다 높습니다.
- 대상 시간이 설정된 아침 ramp 구간입니다.
- 현재 예측이 이미 `recent_same_business_type_mean + base_allowed_excess_mw`를 넘었습니다.

모든 양수 residual을 막는 것은 아닙니다. 예측값이 비영업일 anchor와 허용 폭 안에 있으면 양수 residual은 그대로 통과합니다.
...
전날 평일 lag가 비영업일 anchor보다 과도하게 높고, 당일 오전 실측이 모델 과대예측을 보여주는 토요일 케이스를 단위 테스트로 추가했습니다. 또한 07:44처럼 실측 개수는 충분하지만 마지막 실측 시간이 04시에 머문 handoff 공백에서, 작은 양수 residual이 07~08시 ramp를 끌어올리지 못하는지도 검증합니다.
