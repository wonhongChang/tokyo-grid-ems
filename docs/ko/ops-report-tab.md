...
리포트 생성기는 OpenAI가 문장을 작성하기 전에 fact packet에 부호 방향을 명시합니다.

- `modelErrorMw = modelForecastMw - actualMw`
- `modelBiasMw = mean(modelForecastMw - actualMw)`
- 양수는 과대예측, 즉 예측값이 실측보다 높은 상태입니다.
- 음수는 과소예측, 즉 예측값이 실측보다 낮은 상태입니다.

OpenAI가 이 부호 규칙과 충돌하는 원인 가설을 반환하면, 해당 가설은 폐기하고 deterministic 문구로 fallback합니다.
...
`diagnosticContext`가 있는 리포트는 운영 진단 요약을 함께 표시합니다.

- 확정 실측 커버리지
- 보정 컨트롤러의 기준 보정량
- 예측 밴드 품질
- 발표 예측선 보존 정책으로 인한 영향

이 영역은 일반 사용자에게 긴 내부 로그를 그대로 노출하기보다, 운영자가 하루의 예측 곡선이 왜 그렇게 보였는지 빠르게 확인할 수 있도록 압축된 요약 카드로 구성합니다. Stage attribution과 freeze 관련 세부값은 접을 수 있는 상세 영역에 둡니다.
...
UI에서는 각 후보를 다음처럼 실험 티켓에 가까운 형태로 나눠 표시합니다.
...
따라서 문장은 production command가 아니라 backtest/replay 후보 톤을 유지해야 합니다.
