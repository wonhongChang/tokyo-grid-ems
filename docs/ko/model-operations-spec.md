...
일반 D0 추론은 절대 q50, lag-24 잔차, 비영업일 구조를 유지합니다. source view는 500MW trust region으로 제한됩니다. D-1 행에서 `lag_24h`나 최근 영업 타입 수요가 없으면 LightGBM 결측 분기에 의존하지 않고 축소된 정보 집합의 전용 모델을 사용합니다. 비영업일 specialist는 가중치 1.0입니다.

Same-regime 보정은 JST 전날에 발행하는 D-1 예측에만 적용합니다. 최근 같은 레짐 확정 3일의 일평균 잔차 중앙값에 25%를 곱하고 +/-1,000MW로 제한합니다. 수정 불가능한 D-1 발행본이 있는 잔차만 사용하며, D0 seed와 오래된 같은 레짐 표본은 제외합니다. 다른 레짐의 최신 기록이 이를 대신 통과시키지 못합니다. 당일 D0에는 이 D-1 보정을 재사용하지 않고 기존 intraday 보정을 유지합니다.

최종 p95 target은 최근 28일의 사전 발행본을 리드 구간과 대상 시간대로 나누어 계산합니다. 호환되는 각 집계에는 최소 24표본·4개 날짜가 필요하며, 유효한 같은 레짐/전체 레짐 오차 q95 중 큰 값에 1.05를 곱합니다. 이력 부족, artifact·정책 불일치, 필요 반폭 3,750MW 초과 시 native 밴드의 정규화·상한 처리를 유지합니다. 정책 fingerprint가 없는 과거 발행본은 새 집계를 초기화할 수 없으므로 도입 초기 밴드가 넓어질 수 있습니다. 대체 밴드나 p95 명칭 자체가 실측 포함률 95%를 보장하지는 않습니다. 자세한 조건은 [서빙 보정 계약 개선](model-improvements/model-improvement-2026-09-09-serving-calibration-contracts.md)을 참고합니다.
...
### 관측 입력 공백과 외부 benchmark 맥락

2026-08-21 캐시 감사 범위는 2023-01-01부터 현재 예측 horizon까지 31,897개 시간 행입니다. 기온은 완전했지만 `humidity_pct`, `discomfort_index`, `weather_source`는 전체 이력의 93.0%가 비어 있었고 최근 60일만 세 항목이 완전했습니다. LightGBM은 결측을 처리할 수 있지만, 이 이력으로는 계절을 가로지르는 안정적인 습도 반응을 학습하기 어렵습니다.

| 공백 | 현재 프로젝트 입력 | 더 풍부한 운영 맥락을 보여주는 공개 근거 | 우선순위 |
|---|---|---|---:|
| 시간 해상도 | 시간 단위 수요 | TEPCO AREA 실적 파일은 30분 수요·발전·재생에너지 출력을 제공 | 높음 |
| 기상 공간장 | 도쿄 AMeDAS 한 지점 | 광역 수요는 공급구역 내 지역별 기상 차이에 반응 가능 | 높음 |
| 예보 vintage | 과거 replay에 최종 관측 기상 사용 | 실제 발행 시점에 이용 가능했던 기상 예보가 보존되지 않음 | 최우선 |
| 재생에너지 맥락 | 운영 피처 없음 | AREA 파일에 재생에너지 출력이 있음. 5개 lag 피처 실험은 28일만 개선하고 56/84일을 악화시켜 기각 | 중간, 재설계 필요 |
| 운영 맥락 | 달력과 공휴일 | 산업체 일정, 대형 이벤트, 수요반응, 계통 운영 신호 없음 | 연구 |

TEPCO의 비공개 운영 피처는 알 수 없습니다. 특정 비공개 입력을 쓴다는 설명은 사실이 아니라 추정으로만 다뤄야 합니다. 프로젝트에서 먼저 해결할 수 있는 공백은 발행 시점 기상 vintage 보존이며, 이후 지역 기상과 30분 shape 피처를 같은 28/56/84일 gate로 검증합니다.
...
| `same_day_latest_actual_mw` | 근거리 가드의 증거로 쓰는 당일 최신 실측 수요 |
...
현재 `run_batch.py` 기준 stage 이름은 `raw_lgbm`, `same_regime_level_calibrated`, `analog_adjusted`, `post_holiday_guarded`, `midday_guarded`, `localized_shape_guarded`, `pre_calibration`입니다. Intraday 보정은 `pre_calibration` 이후 실행합니다. 그 내부의 최종 shape/ramp 가드는 `shapeGuardDeltaMw`, `rampGuardDeltaMw`로 별도 기록하므로 이 변동을 잔차 이월량만으로 설명하면 안 됩니다.
...
| 실측 기반 저녁 회복 | 비영업일 양수 잔차 감쇠 | 연속 세 실측 잔차가 양수이고 최신/평균 기울기 600/300MW 및 타깃 과거 지지 0 이상일 때, 마지막 실측에서 3시간 이내의 이 감쇠만 생략 ([9월 21일 계약](model-improvements/model-improvement-2026-09-21-observed-evening-recovery.md)) |
...
| serving calibration | `same_regime_day_level.max_state_lag_days` | 2일 | 발행일 기준 전체 freshness와 일본 영업일·휴일 달력 기준 같은 레짐 표본의 최신성을 함께 확인합니다. 해당 날짜가 빠지면 보정을 생략하며, 한도를 늘리면 오래된 증거를 허용하게 됩니다. |
| serving calibration | `same_regime_day_level.application`, `require_day_ahead_origin` | `day_ahead_only`, true | D-1 발행 잔차를 D-1 예측에만 사용합니다. 별도 리드 일치 검증 없이 D0 적용이나 D0 seed 허용을 복구하지 않습니다. |
...
| interval | `rolling_conformal_floor` | 이전 설정 유지, 목표 포함률 95% | 운영에서는 lead-aware 분기가 이전 final-served 집계보다 우선합니다. 사후 공개선 포함률을 고정 발행본 포함률로 해석하거나 이전 모드를 손쉬운 롤백으로 복구하지 않습니다. |
| served interval | `served_interval_calibration` | `lead_aware_conformal_target_width`, 28일, 24표본, 4개 날짜, safety 1.05, 상한 3,750MW | artifact·q50 정책·리드 구간·시간대를 맞춥니다. 필요 표본을 늘리면 가동이 늦어지고, safety를 낮추면 폭과 포함률이 함께 낮아질 수 있습니다. 표본 부족이나 상한 초과 시 native 폭 유지 상태를 명시합니다. 학습 artifact fingerprint와 별도의 정책입니다. |
...
| intraday | `afternoon_observed_anchor_cap.severe_overforecast` | enabled | 최신 residual과 평균 residual이 모두 크게 음수일 때만 오후 회복 slope veto를 완화합니다. 점심 이후 반등이 있더라도 hot plateau를 cap할 수 있지만, residual 강도와 보수적 slope 상한으로 제한됩니다. |
...
| post-processing | `business_return_anchor_shortfall.observed_overforecast_veto` | `hour>=7`, lead `1..3`, 오차 `>=1200 MW` | 당일 실측이 큰 과대예측을 확인한 뒤 영업일 복귀 추가 리프트만 취소합니다. 오차 임계값을 낮추면 더 자주 개입하고, lead 범위를 늘리면 정상적인 후반 회복까지 누를 위험이 커집니다. |
